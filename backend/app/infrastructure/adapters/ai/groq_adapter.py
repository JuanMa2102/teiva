...
class GroqAdapter(AIPort):
    ...
    async def generate_completion(self, prompt: str, system_instruction: str = "") -> str:
        # Implementación stub de llamada a la API de Groq
        # En producción se importaría `groq.Groq`
        return f"Groq response for: {prompt[:30]}..."

    async def analyze_document(self, file_content: bytes, mime_type: str) -> Dict[str, Any]:
        return {"provider": "groq", "analysis": "Stub analysis data"}
